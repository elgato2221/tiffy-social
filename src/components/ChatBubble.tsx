        ...
        <div className={`max-w-[75%] min-w-0 rounded-2xl overflow-hidden ${isMine ? "rounded-br-md" : "rounded-bl-md"}`}>
          {mediaUnlocked && mediaUrl ? (
            <div className="relative">
              ...
              <div className={`px-3 py-1.5 ${isMine ? "bg-purple-500" : "bg-gray-800"}`}>
                <span className={`text-[10px] ${isMine ? "text-purple-100" : "text-gray-400"}`}>{time}</span>
              </div>
            </div>
          ) : (
            <div className={`${isMine ? "bg-purple-500/80" : "bg-gray-800"} p-4`}>
              <div className="w-full h-40 bg-black/30 rounded-xl flex flex-col items-center justify-center">
                ...
                {!isMine && onUnlockMedia ? (
                  ...
                  <div className="flex items-center gap-1 mt-2">
                    <CoinIcon size="xs" />
                    <span className="text-[11px] text-gray-400">{mediaPrice} moedas</span>
                  </div>
                ) : null}
              </div>
      ...
      >
        {type === "audio" ? (
          <div className="flex items-center gap-2 min-w-[140px] max-w-full">
            ...
            />
            <button onClick={toggleAudio} className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${isMine ? "bg-white/20 hover:bg-white/30" : "bg-gray-700 hover:bg-gray-600"} transition`}>
              {playing ? (
        ...
        <div className={`flex items-center gap-1.5 mt-1 ${isMine ? "justify-end" : "justify-start"}`}>
          <span className={`text-[10px] ${isMine ? "text-purple-100" : "text-gray-400"}`}>{time}</span>
          {cost > 0 && (
            <span className={`text-[10px] flex items-center gap-0.5 ${isMine ? "text-purple-100" : "text-gray-400"}`}>
