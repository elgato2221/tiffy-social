...
export default function LeftSidebar() {
  ...
  return (
    <aside className="hidden lg:flex flex-col w-64 xl:w-72 h-screen sticky top-0 border-r border-gray-200 bg-white px-4 py-6 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      {/* Logo */}
      <Link href="/feed" className="flex items-center gap-2.5 px-3 mb-8">
        ...
        <span className="text-xl font-bold text-gray-900">Tiffy Social</span>
      </Link>
      ...
      {isAdmin && (
        <Link
          href="/admin"
          className={`flex items-center gap-3 rounded-xl px-3 py-2.5 mb-2 text-sm font-semibold transition-colors ${
            pathname === "/admin"
              ? "bg-purple-500/10 text-purple-500"
          ...
          {users.map((user, i) => (
            ...
            >
              <div className="relative flex-shrink-0">
                ...
                {user.online && (
                  <div className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-white bg-purple-400" />
                )}
              </div>
